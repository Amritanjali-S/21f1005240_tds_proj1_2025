...
#response= requests.get("https://discourse.onlinedegree.iitm.ac.in/c/courses/tds-kb/34.json", headers=headers)

# with open("discourse.json","w") as file:
#     json.dump(response.json(), file, indent=4)

def get_topic_ids(base_url, category_slug, category_id, start_date_str, end_date_str, cookie):
    ...
    while True:
        ...
        try:
            response = requests.get(paginated_url, headers=headers, timeout=30) # cookies=cookie
...
def get_full_topic_json(base_url, topic_id, cookie):
    ...
    try:
        response = requests.get(initial_topic_url, headers=headers, timeout=30) #cookies=cookie
        response.raise_for_status()
    ...
    for i in range(0, len(missing_post_ids), POST_ID_BATCH_SIZE):
        ...
        try:
            batch_response = requests.get(posts_url, params=query_params, headers=headers, timeout=60) #cookies=cookie
...
def main():
    ...
    if not cookie and DISCOURSE_BASE_URL != "https://meta.discourse.org/":
    ...
    for i, topic_id in enumerate(topic_ids, 1):
        ...
        topic_json_data = get_full_topic_json(DISCOURSE_BASE_URL, topic_id, cookie)
        if topic_json_data:
